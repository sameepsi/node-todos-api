...
describe('POST /users/login', () => {
  it('Should return valid user and auth token for valid credentials', (done) => {
    supertest(app)
    .post('/users/login')
    .send({
      email:users[1].email,
  ...
  it('Should return 400 for invalid credentials', (done) => {
    supertest(app)
    .post('/users/login')
    .send({email:'sameepsi@gmail.com', password:'sameepsinghania'})
    .expect(400)
    .expect((res) => {
      expect(res.body).toEqual({})
    })
...
describe('DELETE /users/me/token', () => {
  it('Should delete token and return 200 ok', (done) => {
    supertest(app)
